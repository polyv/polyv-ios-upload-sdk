...
PLVVodUploadSDK 是 polyv 公司提供的可集成于项目中用于 iOS 设备上传视频文件到服务器的 SDK，支持断点续传。使用本 SDK 需要在 polyv 公司的[视频云平台](http://www.polyv.net/)注册账号，并使用该账号进行登录，上传后的视频文件可在该视频平台上进行查看、剪辑、播放、删除等操作。
